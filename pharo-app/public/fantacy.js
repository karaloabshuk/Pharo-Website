document.addEventListener('DOMContentLoaded', () => {
    ...
    document.querySelectorAll('.hero-buttons .btn, .cta-section .btn').forEach(btn => {
    ...
    function loadSavedTeam() {
        const raw = localStorage.getItem('pharo_saved_team');
        if (!raw) return;
        try {
            const saved = JSON.parse(raw);
            Object.keys(saved.players || {}).forEach(pos => {
                const p = saved.players[pos];
                selectedPlayers[pos] = { name: p.name, price: p.price, src: p.src };
            });
            totalSpent = saved.totalSpent || 0;
            captainPos = saved.captain || null;
            isSaved = true;
            renderSquadPitch();
            updateBudget();
            updateTransfersBudget();
    ...
    function resetTeam() {
        ...
        renderTransferList();
        renderTransferSquad();
        showToast('Team reset. Start fresh!', 'info');
    }
